...
function Show-DetailedResults {
    ...
    # Test mode results
    if ($Script:TestMode) {
        Write-Log "`nTEST MODE RESULTS:" -Level "TEST" -Color $Colors.Test
        foreach ($section in $Script:Stats.Keys) {
            if ($Script:Stats[$section].TestResults.Count -gt 0) {
                Write-Log "$section:" -Level "TEST" -Color $Colors.Test
                foreach ($result in $Script:Stats[$section].TestResults | Select-Object -First 5) {
                    Write-Log "  $result" -Level "TEST" -Color $Colors.Test
                }
                if ($Script:Stats[$section].TestResults.Count -gt 5) {
                    Write-Log "  ... and $($Script:Stats[$section].TestResults.Count - 5) more" -Level "TEST" -Color $Colors.Test
                }
            }
        }
    }
...
"@
    ...
    if (-not $DryRun -and -not $Script:TestMode) {
        ...
        Write-Log "`nSIMULATION COMPLETED - No changes made" -Level "INFO" -Color $Colors.Success
        Write-Log "Run without -DryRun and -TestMode to execute optimizations" -Level "INFO" -Color $Colors.Success
    }
}
