...
// Use the correct Supabase URL from your environment
...
async function importBusinessData() {
    try {
        ...
        // First, verify we can connect to the correct database
        ...
        if (testError) {
            console.error('Cannot connect to database or table not found:', testError);
            return;
        }
        ...
        if (currentCount > 0) {
            console.log('⚠️  Database already contains data. You may want to clear it first.');
        ...
        // Process businesses in smaller batches to avoid timeouts
        const batchSize = 25;
        for (let i = 0; i < data.length; i += batchSize) {
            ...
            for (const business of batch) {
                try {
                    ...
                    if (error) {
                        console.error(`Error inserting business "${businessData.business_name}":`, error.message);
                        errorCount++;
                    } else {
                        successCount++;
